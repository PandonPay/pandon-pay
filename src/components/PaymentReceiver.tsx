...
const PaymentReceiver = () => {
  const [paymentStatus, setPaymentStatus] = useState<'idle' | 'connecting' | 'pending' | 'success' | 'failed'>('idle');
      ...
      const interval = setInterval(() => {
        setProgress(prev => {
          if (prev >= 100) {
            ...
            setTransactionHash("0x1a2b3c4d5e6f7g8h9i0j1k2l3m4n5o6p7q8r9s0t1u2v3w4x5y6z");
            toast({
              title: "支付成功",
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-lg">{paymentData.amount} {paymentData.currency}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">网络:</span>
                <Badge className="bg-blue-100 text-blue-700">{paymentData.network}</Badge>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">收款方:</span>
                <code className="text-sm bg-gray-100 px-2 py-1 rounded">{paymentData.recipient}</code>
              </div>
              <div className="flex justify-between">
        ...
        <CardContent className="p-6">
          ...
          <div className="space-y-4 mb-6">
            {paymentSteps.map((step, index) => {
              const isActive = index === currentStep && paymentStatus === 'pending';
              const isCompleted = index < currentStep || paymentStatus === 'success';
            ...
            {paymentStatus === 'success' && (
              <div className="text-center space-y-4">
                ...
                <p className="text-sm text-gray-600">
                  交易哈希: <code className="bg-gray-100 px-2 py-1 rounded text-xs">{transactionHash}</code>
                </p>
              </div>
            )}
